...
// makeBasicHost creates a LibP2P host with a random peer ID listening on the given multiaddress. It will use secio if secio is true.
func makeBasicHost(listenPort int, secio bool, randseed int64) (host.Host, error) {
	// If the seed is zero, use real cryptographic randomness. Otherwise, use a deterministic randomness source to make generated keys stay the same across multiple runs
	var r io.Reader
	if randseed == 0 {
		r = rand.Reader
	} else {
...
func main() {
	...
	// LibP2P code uses golog to log messages. They log with different string IDs (i.e. "swarm"). We can control the verbosity level for all loggers with:
	...
	if *target == "" {
		...
		// Make a new stream from host B to host A, it should be handled on host A by the handler we set above because we use the same /p2p/1.0.0 protocol
